...
const NavItem = ({ label, icon, active, onClick }: any) => {
  ...
  return (
    ...
    >
      ...
      {active && (
        <div className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-1 bg-[#FF6900] rounded-r-full shadow-[0_0_8px_#FF6900]" />
      )}

      <svg
        className={`w-5 h-5 transition-colors ${active ? "text-[#FF6900]" : "text-white/40 group-hover:text-white"}`}
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        strokeWidth={2}
